...
export var AuthService = (function() {
  ...
  AuthService.prototype.register = async function(input) {
    ...
    try {
      ...
      var r = await c.query('INSERT INTO users (email, password_hash, name, department) VALUES (,,,) RETURNING id, email, name, role', [input.email, hash, input.name, input.department || null]);
      return r.rows[0];
    } finally { c.release(); }
  };
  AuthService.prototype.login = async function(input, ip) {
    var c = await pool.connect();
    try {
      var r = await c.query('SELECT * FROM users WHERE email =  AND is_active = true', [input.email]);
